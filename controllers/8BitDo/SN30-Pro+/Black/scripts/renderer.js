...
const starButtonImage = document.getElementById('star-button');
...
function checkGamepad() {
    ...
    if (gamepad) {
        ...
        if (isVendorProductMatch(vendorId, productId, "045e", "028e") | isVendorProductMatch(vendorId, productId, "045e", "02e0")) {
            handleXbox(gamepad);
            detectedMode = "XBox";
        } else if (isVendorProductMatch(vendorId, productId, "2dc8", "6102")) {
            handleAndroid(gamepad);
            detectedMode = "Android/Apple";
        } else {
    ...
    requestAnimationFrame(checkGamepad);
    document.title = `8BitDo Black (${detectedMode} mode)`;
}
