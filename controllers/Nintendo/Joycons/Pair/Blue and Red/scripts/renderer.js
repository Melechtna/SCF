...
function checkGamepad() {
    ...
    if (gamepad) {
        ...
        if (vendorId === desiredVendorId && productId === desiredProductId) {
            ...
            const isSLLeftPressed = gamepad.buttons[18].pressed;
            updateSLLeftButton(isSLLeftPressed);

            // Check SR Left button
            const isSRLeftPressed = gamepad.buttons[19].pressed;
            updateSRLeftButton(isSRLeftPressed);

            // Check SR Right button
            const isSRRightPressed = gamepad.buttons[21].pressed;
            updateSRRightButton(isSRRightPressed);

            // Check SL Right button
            const isSLRightPressed = gamepad.buttons[20].pressed;
